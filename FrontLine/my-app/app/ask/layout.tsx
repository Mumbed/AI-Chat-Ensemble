    ...
    const roomAsync = async () => {
      ...
      if (!userDataResource.isLogined) {
        ...
        if (!roomDataSource.success) {
          ...
          setList(roomDataSource.data.map((item: { question: string }) => item.question));
        }
      } else {
        setList(userDataResource.rooms);
      }
      ...
      <div className="flex-1 flex flex-col">
        <section className="flex-1 flex items-center justify-center">
          <div className="inline-block text-center justify-center">
            <RoomContext.Provider value={refreshState}>
              {isLoading ? (
                <CircularProgress label="답변 생성중...." className="" />
              ) : (
                children
              )}
